...
def run_tinygo():
    ...
    if len(sys.argv) <= 1:
        return subprocess.call(tinygo, stdout=sys.stdout, stderr=sys.stderr, shell=True)
        ...
        return subprocess.call(cmd, stdout=sys.stdout, stderr=sys.stderr, shell=True)
    else:
        cmd = sys.argv[:]
        cmd[0] = tinygo
        return subprocess.call(cmd, stdout=sys.stdout, stderr=sys.stderr, shell=True)
...
def run_eosio_cpp():
    ...
    eosio_cpp = os.path.join(dir_name, 'eosio.cdt/bin/eosio-cpp')
    cmd = sys.argv[:]
    cmd[0] = eosio_cpp
    return subprocess.call(cmd, stdout=sys.stdout, stderr=sys.stderr, shell=True)
